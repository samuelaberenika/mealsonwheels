from flask import Flask, render_template , request, session, redirect, url_for, flash
import sqlite3

import os
from werkzeug.utils import secure_filename
UPLOAD_FOLDER = '/Macintosh HD/Users/samuelaberenika/Desktop/'
ALLOWED_EXTENSIONS = {'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif'}

app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER

app = Flask(__name__)

@app.route ('/', methods=['GET', 'POST'])
def login():
    if request.method == 'POST':
        username = request.form.get('username')
        password_plaintext = request.form.get('password')

        db = sqlite3.connect("database.sql")
        cursor = db.cursor()

        result = cursor.execute("SELECT username FROM user WHERE username=? AND password=?", (username, password_plaintext))
        data = result.fetchone()
        db.close()

        if data != None:
            session['user'] = username
            return redirect(url_for('HomePage'))
        else:
            return redirect(url_for('login'))
    else:
        return render_template('Login.html')
    
    
@app.route ('/Signup', methods=['GET', 'POST'])
def signup():
    if request.method == 'POST':
        username = request.form.get('username')
        password_plaintext = request.form.get('password')
        confirm_password_plaintext = request.form.get('confirmpassword')

        if password_plaintext!= confirm_password_plaintext:
            flash("Passwords do not match.")
            return render_template('signup.html')

        try:
            db = sqlite3.connect("database.sql")
            cursor = db.cursor()

            result = cursor.execute("INSERT INTO users", (username, password_plaintext, confirm_password_plaintext))
            db.close()
        except sqlite3.Error as error:
            flash("Username invalid.")
            return render_template('signup.html')
    else:
        return render_template('Signup.html')
    




@app.route ('/HomePage.html')
def HomePage():
    if 'user' in session:
        return render_template('HomePage.html')
    else:
        return redirect(url_for('login'))

@app.route ('/vendor.html')
def Foodvendor():
    return render_template('vendor.html')


@app.route ('/donor.html')
def donor():
    return render_template('donor.html')

@app.route ('/logistics.html', methods=['GET', 'POST'])
def logistics():
    if request.method == 'POST':
        if 'license' not in request.files:
            flash('No file part')
            return redirect(url_for('logistics'))
        print('file found...')
        file = request.files['license']
        file.save(os.path.join(app.config['UPLOAD_FOLDER'], file))
    else:
        return render_template('logistics.html')

@app.route ('/Login.html')
def logout():
    del(session['user'])
    return redirect(url_for('HomePage'))





#app.secret_key = "milf"
#app.run(host= '0.0.0.0', port=8080, debug=True)

if __name__ == '__main__':
    app.secret_key = "milf"
    app.run(host='0.0.0.0', port=8080, debug=True)





